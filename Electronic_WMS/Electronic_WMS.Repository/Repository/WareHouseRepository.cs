...
namespace Electronic_WMS.Repository.Repository
{
    public class WareHouseRepository : IWareHouseRepository
    {
        ...
        {
            return _db.WareHouseEntities.Where(x => x.Status == (int)CommonStatus.IsActive && x.Name.ToLower() == name.ToLower()).FirstOrDefault();
        }

        public IEnumerable<WareHouseEntity> GetList()
        {
            return _db.WareHouseEntities.Where(x => x.Status == (int)CommonStatus.IsActive).ToList();
        }
